# Scripts/rerank_lightglue.py — LightGlue+SuperPoint 재정렬 (per-env 저장, tqdm, 타임스탬프/롤업)

### ================================ 사용방법 ================================ 
# light2025 환경(또는 --profile light), visdrone
# (light2025) knk2025@DESKTOP-59ULDOH:/mnt/d/KNK/_KSNU/_Projects/dino_test/scripts$ python rerank_lightglue_sp.py --dataset visdrone --k 20 --alpha 0.7 --device cuda

# 경로 수동 + 프로파일 강제
# python rerank_lightglue_sp.py --index-dir D:/.../data/index/visdrone --queries-dir D:/.../data/corpora/visdrone/images --k 20 --alpha 0.7 --profile light --device cuda

### ================================ 사용방법 ================================ 

# 저장 규칙

# 출력 폴더: results/<dataset>/<profile>/light/tables/

# 스냅샷(ts): search_results_rerank_light_<YYYYmmdd-HHMMSS>.csv, rerank_perf_summary_light_<YYYYmmdd-HHMMSS>.csv

# latest 별칭: search_results_rerank_light.csv, rerank_perf_summary_light.csv

# 누적 로그: rerank_light_runs.csv
...
# GT: data/gt/<dataset>/gt_queries.csv (없으면 eval_search.py로 생성)

# Scripts/rerank_lightglue.py — LightGlue+SuperPoint 재정렬
# (게이트 + α결합 + Top-K, per-env 저장, tqdm, 타임스탬프/롤업)
#
# 예)
#   (light2025) python rerank_lightglue.py --dataset visdrone --k 20 --alpha 0.2 --device cuda
#   python rerank_lightglue.py --index-dir D:/.../data/index/visdrone \
#       --queries-dir D:/.../data/corpora/visdrone/images --k 20 --alpha 0.2 --profile light --device cuda
...
def lg_score(img0, img1, extractor, matcher, device, ransac_thresh=5.0):
    """
    반환:
      score         : 휴리스틱 점수(로그용)
...
# ---------------- main ----------------
def main():
    ap = argparse.ArgumentParser(description="Rerank with LightGlue+SuperPoint on FAISS Top-K candidates.")
    ...
    ap.add_argument("--device", type=str, default="auto", choices=["auto","cuda","cpu"], help="임베딩/LightGlue 디바이스 힌트")
    # LG/SP 파라미터
    ...
    stage   = "light"  # LightGlue 리랭크 스테이지 고정
    ...
    print(f"[INFO] dataset={dataset}, profile={profile}, stage={stage}, index.ntotal={getattr(index,'ntotal',0)}, K={K}, alpha={alpha}, LG device={lg_device.type}")

    out_rows = []
    embed_ms_all, search_ms_all, geom_ms_all, total_ms_all = [], [], [], []
    rerank_applied_cnt, rerank_k_sum = 0, 0

    pbar_q = tqdm(gt_rows, desc=f"Rerank LightGlue+SP ({dataset})", unit="q", ncols=100)
    for row in pbar_q:
        ...
        # 3) LightGlue+SP 기하 매칭 → inlier_ratio(0..1)
        try:
        ...
        for p in pbar_c:
            try:
                ...
                s, inl, ir = lg_score(qimg, dimg, extractor, matcher, lg_device, ransac_thresh=args.ransac)
            except Exception:
                s, inl, ir = 0.0, 0, 0.0
            geom_scores_raw.append(s)   # 로깅용
        ...
        out_rows.append({
            ...
            "top1_geom_raw": f"{geom_raw2[0]:.6f}",    # 휴리스틱 점수(참고)
